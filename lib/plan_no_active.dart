...
// import '/Users/danie/Documents/DLEConseil/Developpement/intence/dev/flutter/packages/ifp_widgets/lib/src/webview.dart';
...
class _PlanWithWebcams extends State<IFWPlanWithWebcams> {
  ...
  void _onInteractionStart(ScaleStartDetails interaction) {
    ...
    // print("_onInteractionStart $interaction");
  }
  ...
  void _onInteractionEnd(ScaleEndDetails interaction) {
    ...
    // print("_onInteractionEnd $interaction $_image"
    //     " ${_transformationController.toScene(const Offset(0.0, 0.0)).dx}"
    //     " ${interaction.velocity.pixelsPerSecond.distance} "
    //     " $isGesturePan");
  }
...
class _PlanWithWebcamsDelegate extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    // print(
    //     "-----------------------------size= $size $isInteraction $isInteractionEnd");
    // set taille image
    layoutChild(
      "-1",
      BoxConstraints(maxHeight: size.height, maxWidth: size.width),
    );
    // positionChild("-1", const Offset(0.0, 0.0));
  ...
  List<List<double>> _calculatePositionWidgets(
    ...
    // print(
    //     "pos-infos ${parameters.imageWidth} ${parameters.imageHeight} $scaleX $scaleY $sceneX $sceneY $screenWidth $screenHeight");
    for (int i = 0; i < webcams.length; i++) {
      position = _position(
          sceneX,
          sceneY,
      ...
      widgetPositions[i][3] = position.dy +
          ((webcams[i]["selected"])
              ...
              scaleY;
      // print(
      //     "pos-webcam $i ${webcams[i]["Plan_XY"][0]}/${webcams[i]["Plan_XY"][1]} ${widgetPositions[i][0]}/${widgetPositions[i][1]} ${widgetPositions[i][2]}/${widgetPositions[i][3]}");
    }
